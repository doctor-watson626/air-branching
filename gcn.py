...
class BipartiteGraphConvolution(torch_geometric.nn.MessagePassing):
    ...
    def __init__(self, norm=torch.nn.LayerNorm):
        super().__init__("add")
        emb_size = 64
        ...
        self.post_conv_module = torch.nn.Sequential(norm(emb_size))
    ...
    def forward(self, left_features, edge_indices, edge_features, right_features):
        ...
        output = self.propagate(
            edge_indices, # edge_indices的结构[2, num_messages]
...
class GNNActor(torch.nn.Module):
    def __init__(self,args,norm=torch.nn.LayerNorm):
        super().__init__()
        self.args = args
        emb_size = 64
